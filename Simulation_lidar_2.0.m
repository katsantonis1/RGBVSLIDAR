...
big_item_z=26.6;
iii=0;
%for small_moving_x=[44,88,33,66,52.8,79.2,44,66,56.4,75.2,49.5,66,58.4,66.6,52.8,66,60,72,55,66,60.9,71,56.5,65.9,61.6,70.4,57.7,66,62.2,70,59.2,66.6,63,70]

for big_moving_x=[37.8,75.6,28.3,56.7,45.4,68,37.8,56.7,48.6,64.8,42.5,56.7,50.4,63,45.3,56.7,51.6,61.8,47.3,56.7,52.4,61.1,48.6,56.7,52.9,60.5,49.7,56.8,53.4,60,50.4,56.7,53.7,59.7]
...
%% Small book is comemnted out to do the big book (line 129)
% for c= round(small_moving_x*scale):1:round((small_moving_x+small_item_x)*scale)
%     for i= global_matrix_y- (round((small_moving_y+small_item_y)*scale)):1:global_matrix_y-round(small_moving_y*scale)
%         global_matrix(i,c)=round(scale*small_item_z);  %We assign ones to the cells where the box belongs in the matrix
%     end
% end

%% Large book
for c= round(big_moving_x*scale):1:round((big_moving_x+big_item_x)*scale)
    for i= global_matrix_y- (round((big_moving_y+big_item_y)*scale)):1:global_matrix_y-round(big_moving_y*scale)
        global_matrix(i,c)=round(scale*big_item_z);  %We assign ones to the cells where the box belongs in the matrix
    end
end
...
colormap('hot');
contourf(xi,yi,contour_matrix,40)


axis equal


sim = sprintf('sim_%d.png', iii);
